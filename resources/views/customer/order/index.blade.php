...
@section('content')
    <div class="container">
        ...
        @foreach ($order_detail as $item)
            <div class="mt-3">
                <h4>{{ $item->produk->nama_produk }} | {{ $item->order->status_pemesanan }}</h4>
                <p>harga Satuan : {{ $item->harga }}</p>
                <p>{{ $item->order->status_pemesanan }}</p>
        ...
        {{-- <ul class="nav nav-tabs" id="myTab" role="tablist">
            ...
            <li class="nav-item">
                <a class="nav-link" id="belum-bayar-tab" data-toggle="tab" href="#belum-bayar" role="tab"
                    aria-controls="belum-bayar" aria-selected="false">Belum Bayar @if (count($order_detail) != 0 and $item->order->status_pemesanan == 'Belum Dibayar')
                        <span class="badge badge-primary">{{ count($order_detail) }}</span>
                    @endif</a></a>
            </li>
            <li class="nav-item">
                <a class="nav-link" id="dikemas-tab" data-toggle="tab" href="#dikemas" role="tab"
                    aria-controls="dikemas" aria-selected="false">Dikemas @if (count($order_detail) != 0 and $item->order->status_pemesanan == 'Sedang Dikemas')
                        <span class="badge badge-primary">{{ count($order_detail) }}</span>
                    @endif</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" id="dikirim-tab" data-toggle="tab" href="#dikirim" role="tab"
                    aria-controls="dikirim" aria-selected="false">Dikirim @if (count($order_detail) != 0 and $item->order->status_pemesanan == 'Sedang Dikirim')
                        <span class="badge badge-primary">{{ count($order_detail) }}</span>
                    @endif</a></a>
            </li>
            <li class="nav-item">
                <a class="nav-link" id="selesai-tab" data-toggle="tab" href="#selesai" role="tab"
                    aria-controls="selesai" aria-selected="false">Selesai @if (count($order_detail) != 0 and $item->order->status_pemesanan == 'Selesai')
                        <span class="badge badge-primary">{{ count($order_detail) }}</span>
                    @endif</a></a>
            </li>
        </ul>
